...
import { View, Text, TouchableOpacity, FlatList, StyleSheet, Image, Alert, TextInput, KeyboardAvoidingView, Platform, Pressable, Modal, Button } from 'react-native';
import { AntDesign, MaterialIcons } from '@expo/vector-icons';
import { collection, doc, getDoc, getDocs, updateDoc, arrayUnion, arrayRemove, onSnapshot, addDoc, deleteDoc, serverTimestamp, query, where, orderBy, document, setDoc, listAll } from 'firebase/firestore';
...
const GroupChatScreen = ({ route }) => {
        ...
        const fetchGroupData = async () => {
            try {
                ...
                if (groupDocSnapshot.exists()) {
                    setGroupData(groupDocSnapshot.data());// Nếu tài liệu tồn tại, hàm setGroupData sẽ được gọi để cập nhật trạng thái với dữ liệu của nhóm.
                } else {
                    console.error('Nhóm không tồn tại.');
                }
            } catch (error) {
                console.error('Lỗi khi tìm nạp dữ liệu nhóm: ', error);
            }
        };

        fetchGroupData();//lấy dữ liệu nhóm khi stateGroupId thay đổi.
    }, [stateGroupId]);
    ...
    const outGroup = async () => {
        try {
            const currentUser = auth.currentUser;
            if (!currentUser) {
                console.error('Hiện tại không có người dùng nào đăng nhập.');
                return;
            }
    
            if (!groupData) {
                console.error('Dữ liệu nhóm chưa được tải.');
                return;
            }
    ...
    // Lưu dữ liệu khi đã đổi thông tin nhóm
    const saveChanges = async () => {
        try {
            ...
            console.log('Tài liệu được viết thành công!');
            toggleModal(); // Đóng modal sau khi lưu thay đổi
        } catch (error) {
            console.error('Lỗi viết tài liệu: ', error);
        }
    };

    
//Cập nhật trạng thái của chat nhóm
    useEffect(() => {
    ...
    //Hiển thị tên nhóm trong group Chat
    ...
    useEffect(() => {
        ...
        // Lấy danh sách bạn bè và xem để user có là thành viên nhóm không
        ...
        const fetchFriendList = async () => {
            try {
                ...
                if (currentUser) {
                    ...
                    const friendDocs = await Promise.all(friendIds.map(friendId => getDoc(doc(db, 'users', friendId))));
                    const friends = friendDocs.map(doc => ({ id: doc.id, ...doc.data(), isSelected: false }));
            ...
            headerRight: () => (
                <TouchableOpacity style={{ marginRight: 10 }}>
                    <View style={{ flexDirection: 'row', marginRight: 20 }}>
                        ...
                        <TouchableOpacity onPress={handleDeleteGroup} style={{ marginLeft: 15 }}>
    ...
    const handleAddUserToGroup = async (userId) => {
        const isUserAdded = friendList.find(user => user.id === userId && user.isSelected);//Tìm user bạn bè
        if (!isUserAdded) {
            try {
                const groupDocRef = doc(db, 'groups', groupId);
                await updateDoc(groupDocRef, { members: arrayUnion(userId) });
                Alert.alert('Thành công', 'Người dùng đã được thêm vào nhóm thành công!');
    ...
    const handleRemoveUserFromGroup = async (userId) => {
        try {
            ...
            if (groupData.creatorId === currentUser.uid) {
                Alert.alert(
                    ...
                    [
                        { text: 'Hủy bỏ', style: 'cancel' },
                        {
                            ...
                            onPress: async () => {
                                try {
                                    await updateDoc(groupDocRef, { members: arrayRemove(userId) });
                                    Alert.alert('Thành công', 'Xóa người dùng ra khỏi nhóm thành công!');
                                    ...
                                    console.error('Lỗi chưa xóa được người dùng ra khỏi nhóm:', error);
                                }
                            }
                        }
                    ]
                ...
                Alert.alert('Không thể xóa admin', 'Bạn không thể xóa admin ra khỏi nhóm.');
            }
        } catch (error) {
    ...
    const handleSendMessage = async (imageUrl, isImage = false) => {
        try {
            ...
            if (!senderName) {
                console.error("Tên người dùng không được xác định trong tài liệu.");
                return;
            }
            ...
            if (!groupName) {
                console.error("Tên nhóm không được xác định trong tài liệu.");
                return;
            }
            ...
            const messageRef = await addDoc(collection(db, 'groupChats'), messageData);
    ...
    const handleSendImage = async () => {
        try {
            ...
            if (status !== 'granted') {
                console.log('Quyền truy cập thư viện phương tiện bị từ chối');
                return;
            }
            console.log('Đã cấp quyền, mở thư viện hình ảnh...');
        ...
        if (currentUser) {
            try {
                ...
                if (groupData.creatorId === currentUser.uid) {
                            ...
                            { text: 'Hủy', style: 'cancel' },
                            {
                                ...
                                onPress: async () => {
                                    try {
                                        ...
                                        Alert.alert('Thành công', 'Nhóm đã được giải tán thành công!');
                                        navigation.goBack();
                                    } catch (error) {
                    ...
                    Alert.alert('Không có quyền', 'Chỉ Admin mới có thể giải tán nhóm này.');
                }
            } catch (error) {
                console.error('Lỗi khi kiểm tra quyền xóa nhóm:', error);
                Alert.alert('Lỗi', 'Không thể kiểm tra quyền xóa nhóm. Vui lòng thử lại.');
            }
        } else {
    ...
    const handleDelete = async () => {
        try {
            if (!selectedMessage || !selectedMessage.id) {
                console.log("Không có tin nhắn nào được chọn để thu hồi");
                return;
            }
            ...
            console.log(`Attempting to delete message with ID: ${messageId} from collection: ${collectionName}`);

            await deleteDoc(docRef);
            console.log("Đã xóa tin nhắn khỏi Firestore");
    ...
    // Menu để chọn các chức năng như xóa tin nhắn
    ...
    return (
        <View style={styles.container}>
                    ...
                    renderItem={({ item }) => (
                        <TouchableOpacity style={styles.userItem} onPress={() => handleAddUserToGroup(item.id)}>
                            ...
                            <View style={styles.userInfoContainer}>
                                ...
                                {item.isSelected ? (
                                    <TouchableOpacity onPress={() => handleRemoveUserFromGroup(item.id)}>
            ...
            <Pressable style={styles.container} onPress={() => setSelectedMessage(null)}>
                <FlatList
            ...
            <KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : "height"} style={styles.inputContainer}>
                ...
                <TextInput
                    ...
                    placeholder={replying ? "Nhập tin nhắn trả lời..." : "Nhập tin nhắn của bạn..."}
                    onChangeText={text => replying ? setReplyContent(text) : setMessage(text)}
                    value={replying ? replyContent : message}
                />
                <TouchableOpacity style={styles.sendButton} onPress={replying ? handleSendReply : handleSendMessage}>
            ...
            >
                <View style={{ flex: 1, justifyContent: 'center', alignItems: 'center', backgroundColor: 'rgba(0, 0, 0, 0.5)' }}>
                    <View style={{ backgroundColor: 'white', padding: 30, borderRadius: 10, alignItems: 'center' }}>
                        <Image source={groupImage} style={{ width: 170, height: 170, marginBottom: 10 }} />
