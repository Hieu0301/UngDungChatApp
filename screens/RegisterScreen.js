...
const RegisterScreen = () => {
  ...
  const registerUser = async () => {
    try {
      ...
      let photoURL = "https://gravatar.com/avatar/94d45dbdba988afacf30d916e7aaad69?s=200&d=mp&r=x";
      if (avatarURI) { 
        try {
      ...
      console.log("Saving user document to Firestore with photo URL:", photoURL);
      const docRef = doc(db, "users", userUID);
      await setDoc(docRef, {
        photoURL: photoURL,
        name: name,
