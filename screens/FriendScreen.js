    ...
    useEffect(() => {
        const fetchUserList = async () => {
            try {
                ...
                const usersNotInFriendList = allUsers.filter(user => user.id !== currentUserUid);
    ...
    const handleToggleFriend = async (userId, userName, isFriend) => {
        try {
            ...
            if (isFriend) {
                ...
                await updateDoc(userDocRef, { friendRequests: arrayUnion(currentUser.uid) });
                const updatedUsers = userList.map(user => {
                    if (user.id === userId) {
    ...
    const renderItem = ({ item }) => (
        <View style={styles.userItem}>
            ...
            <TouchableOpacity
                ...
                style={[styles.addButton, { backgroundColor: item.isFriend ? '#4caf50' : item.friendRequestSent ? '#cccccc' : '#007bff' }]}
                disabled={item.friendRequestSent}
            >
                <Text style={styles.addButtonText}>{item.isFriend ? 'Bạn bè' : item.friendRequestSent ? 'Đã gửi' : 'Kết bạn'}</Text>
            </TouchableOpacity>
        </View>
    );
